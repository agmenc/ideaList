/*
 ...
 See: http://svn.tiddlywiki.org/Trunk/core/jquery/plugins/jQuery.twFile.js
    ...
    getDriver: function() {
        if (this.currentDriver === null) {
            for (var t = 0; t < this.driverList.length; t++) {
                var driverName = this.driverList[t];
                var potentialDriver = drivers[driverName];
                if (this.currentDriver === null && potentialDriver.isAvailable && potentialDriver.isAvailable()){
                    this.currentDriver = potentialDriver;
                    break;
                }
            }
        }
...
drivers.tiddlySaver = {
    ...
    deferredInit: function() {
        if (!document.applets["TiddlySaver"] && !jQuery.browser.mozilla && !jQuery.browser.msie && document.location.toString().substr(0, 5) == "file:") {
            jQuery(document.body).append("<applet style='position:absolute;left:-1px' name='TiddlySaver' code='TiddlySaver.class' archive='TiddlySaver.jar' width='1' height='1'></applet>");
        }
    },
    isAvailable: function() {
